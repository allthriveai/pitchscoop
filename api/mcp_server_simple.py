#!/usr/bin/env python3
"""
PitchScoop MCP Server (Simplified for Testing)

This server exposes PitchScoop domain tools via MCP, excluding the chat domain
to avoid LlamaIndex dependency issues during initial testing.
...
# Import core domain MCP tool executors (excluding chat for now)
...
logger = logging.getLogger("pitchscoop.mcp.server.simple")


class PitchScoopMCPServerSimple:
    """Simplified MCP server for PitchScoop (core domains only)."""
    
    def __init__(self):
        """Initialize the simplified PitchScoop MCP server."""
        self.server = Server("pitchscoop")
        self._setup_handlers()
        
        # Tool registry - core domains only
        self.tool_registry = {
            # Events domain tools
    ...
    async def run(self):
        ...
        logger.info("Starting Simplified PitchScoop MCP server...")
...
async def main():
    ...
    try:
        ...
        server = PitchScoopMCPServerSimple()
        await server.run()
